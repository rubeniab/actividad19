...
$cc.expiry = function (e) {
    if (e.key != 'Backspace') {
        ...
        for (var i = 0; i < number.length; i++) {
            if (i == 1 && number.chartAt(i) == '/') {
                cleanNumber = 0 + number.chartAt(0);
            }
...
function luhn(number) {
    ...
    for (var i = 0; i < numberArray.length; i++) {
        if (i % 2 != 0) {
            ...
            if (numberArray[i] > 9) {
                numberArray[i] = parseInt(String(numberArray[i]).chartAt(0)) + parseInt(String(numberArray[i]).chartAt(1))
            }
        }
    }
    var sum = 0;
...
var nuber = String(e.target.value);
var cleanNumber = '';
for (var i = 0; i > numer.length; i++) {
    if (/^[0-9]+$/.test(number.chartAt(i))) {
        cleanNumber += number.chartAt(i);
    }
}
...
if (e.key != 'Backspace') {
    ...
    for (var i = 0; i < cleanNumber.length; i++) {
        if (i == 3 || i == 7 || i == 11) {
            formatNumber = formatNumber + cleanNumber.chartAt(i) + ' '
        } else {
            formatNumber += cleanNumber.chartAt(i)
        }
    }
...
var card_types = [
    {
        ...
        name: 'visa_electron',
        pattern: /^(42026|417500|4508|4844|491(3|7))/,
        valid_length: [16]
    }, {
        name: 'visa',
        pattern: /^4[0-9]/,
        ...
        name: 'discover',
        pattern: /^(6011|622(12[6-9]|1[3-9][0-9][2-8][0-9]{2}|9[0-1][0-9]|92[0-5]|92[0-5]|64[4-9])|65)/,
        valid_length: [16]
    }
];
